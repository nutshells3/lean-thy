...
section \<open>Substantive axiom packages on agents and targets\<close>

text \<open>
  Substantive locale packages on the Step 1 records:

    \<^item> defeasible source-side derivability at a world;
    \<^item> monotone target-side derivability at a world.

  Their combination yields exact envelope-translation failure directly on
  stochastic agents and formal systems.
\<close>
...
section \<open>Canonical content-sensitive instance: Tweety to monotone logic\<close>
...
section \<open>Positive monotone counterpart\<close>
...
text \<open>
  Main packages and consequences:

    \<^item> @{locale defeasible_nl_agent};
    \<^item> @{locale standard_monotone_target};
    \<^item> @{locale defeasible_nl_to_monotone_target};
    \<^item> Tweety as negative instance;
    \<^item> Monobird as positive counterpart;
    \<^item> @{thm substantive_axioms_separate_translation_outcomes}.
\<close>

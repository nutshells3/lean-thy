...
section \<open>Substantive hidden-key ambiguity packages\<close>

text \<open>
  Finite admissible conflicting key families for one utterance. The main
  consequences are single-shot impossibility, the uniform bound
  @{term "1 / card K"}, and key-revealed recovery.
\<close>
...
section \<open>Canonical ambiguity instance\<close>
...
text \<open>
  Main packages and consequences:
    ...
    \<^item> @{locale ambiguous_nl_source}: uniform bound and key-revealed
      recovery;
    \<^item> @{locale genuinely_ambiguous_nl_source}: single-shot impossibility
      and strict subperfect success;
    \<^item> @{term I_amb}: canonical two-key instance.
\<close>

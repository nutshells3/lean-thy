...
section \<open>Main claims after Step 5\<close>

text \<open>
  Consolidated theorem-backed claims:

    \<^item> substantive source/target axioms separate exact envelope outcomes;
    \<^item> hidden-key ambiguity yields quantitative single-shot bounds and
      recovery under key revelation;
    \<^item> checker-linked verified-search bounds are developed in the next
      theory.
\<close>

section \<open>Claim 1: substantive axioms decide the envelope outcome\<close>

text \<open>
  Same target, different substantive source-side axioms, opposite envelope
  outcomes. No general independence claim is asserted here.
\<close>
...
section \<open>Claim 2: hidden-key ambiguity gives a quantitative ceiling\<close>

text \<open>
  Finite-support prior bound by @{term max_supported_key_mass}; uniform bound
  by @{term "1 / card K"}; both bounds are tight; key revelation restores
  translation on the same utterance.
\<close>
...
text \<open>
  Claim 1 states substantive outcome separation.
  Claim 2 states ambiguity bounds and recovery.
  Checker-linked verified-search bounds are stated in Step 7.
\<close>
